...
class ChemFlow(nn.Module):
    def __init__(self, input_dim, edge_dim, hidden_dim, output_dim,
        ...
        self.set2set2 = Set2Set(543 , processing_steps=2)
        ...
        self.c3_gamma = nn.Sequential(nn.Linear(3, hidden_dim*2), nn.ReLU(), nn.Linear(hidden_dim*2, hidden_dim*2))
        self.c3_beta  = nn.Sequential(nn.Linear(3, hidden_dim*2), nn.ReLU(), nn.Linear(hidden_dim*2, hidden_dim*2))
        self.c4_gamma = nn.Sequential(nn.Linear(3, 202), nn.ReLU(), nn.Linear(202, 202))
        self.c4_beta  = nn.Sequential(nn.Linear(3, 202), nn.ReLU(), nn.Linear(202, 202))
    ...
    def forward(self, data):
        ...
        expanded_x1 = expanded_x
        per_mol_cross, mix_feat,expanded_x = self.cross_group_attn(
            xg_list, gb_list,
            expanded_x_flat=expanded_x
        )

        expanded_x_update = torch.empty((batch_size * K, 220), dtype=s1.dtype, device=device)


        expanded_x_update[0::K] = expanded_x[:, 0, :]; expanded_x_update[1::K] = expanded_x[:, 1, :]; expanded_x_update[2::K] = expanded_x[:, 2, :]

        expanded_x = torch.cat((expanded_x1,expanded_x_update, global_node_attr), dim=1)

        batch_vec = torch.repeat_interleave(torch.arange(batch_size, device=device), K)

        set2set_x = self.set2set2(expanded_x, batch_vec)
        ...
        cross_stack = gamma4 * cross_stack + beta4

        group = torch.cat((group,cross_stack,global_node_attr), dim=1)
