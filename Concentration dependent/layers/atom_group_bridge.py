...
class AtomGroupBridgeFiLM(nn.Module):
    def __init__(self, atom_dim, group_dim, cond_dim, hidden=180,s2s_steps: int = 2):
        ...
        self.g_proj = nn.Linear(47, group_dim-80-41)
        self.s2s_a2g = Set2Set(80, processing_steps=s2s_steps)   #  2*Dg
        self.merge_a2g = nn.Linear(group_dim, group_dim)            # 2*Dg -> Dg


        self.s2s_g2a = Set2Set(group_dim, processing_steps=s2s_steps)   #  2*Dg
    ...
    def forward(self, x_atom, atom_idx, x_group, group_idx, edge_index_group, cond_atom,edge_attr_group=None):
        ...
        xg = self.merge_a2g(xg_a2g)  # [Gm, Dg]# [Gm, 2*Dg]
        ...
        xg = torch.cat((x_group,xg,cond_g),dim=1)
